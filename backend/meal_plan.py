...
- `summary` 는 **왜 이 조합인지**를 두세 문장(160자 안쪽)으로. 아래 셋을
  반드시 담아라 — 사용자는 이 말을 보고 크레딧을 쓸 값어치가 있었는지 판단한다.
  1) 냉장고에 있는 것 중 **무엇을 주로 썼는지**
  2) **새로 사야 하는 것이 무엇이고, 그것을 몇 번 나눠 쓰는지**
     ("애호박 하나로 세 끼" 처럼. 이게 이 기능의 요점이다 — 싱싱한 재료를
     몇 개만 사서 여러 날에 나눠 쓰면 돈도 덜 들고 남아서 버리지도 않는다)
  3) 사용자가 적은 조건을 **어떻게 반영했는지**
  "맛있는 식단이에요" 처럼 아무 판단도 안 든 말은 쓰지 마라. 광고 문구도 안 된다.
  예: "냉장고의 감자·달걀을 중심으로 짰어요. 새로 살 건 애호박·두부 둘뿐인데
  각각 세 끼에 나눠 쓰도록 골라서 남기지 않아요. 아이가 먹기 좋게 맵지 않은
  것만 담았습니다."

아래 JSON 객체만 출력해라. 다른 텍스트는 출력하지 마라.
예: {{"summary": "냉장고의 감자를 세 번 나눠 쓰고, 새로 살 건 두부 하나예요", "plan": [{{"n": 3, "why": "양파가 이틀 남았어요"}}, {{"n": 11, "why": "재료가 다 있어요"}}]}}
"""
