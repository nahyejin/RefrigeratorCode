...
- **`새로 살 것 없음` 인 것을 먼저 고른다.** 이 식단의 값어치는 "몇 개만 사면
  일주일" 에 있다. 그것으로 {days}개가 채워지면 끝낸다.
- 새로 사야 하는 것이 있다면, **이미 고른 요리가 사는 것과 겹치는 쪽**을 골라라.
  애호박을 사야 하는 요리를 이미 하나 골랐다면, 애호박을 쓰는 다른 요리는 공짜다.
  하나 사서 여러 날 쓰는 것이 이 기능의 요점이다.
...
- `summary` 는 **사용자가 적은 조건을 어떻게 읽었는지** 한 문장(80자 안쪽).
  그것만 써라.
  **숫자를 쓰지 마라** — 몇 개를 사야 하는지, 며칠치인지, 몇 끼에 나눠 쓰는지는
  네가 고른 뒤에 서버가 센다. 네가 숫자를 말하면 화면에 뜨는 것과 어긋난다
  (실제로 "0개짜리만 엄선했다" 고 해 놓고 장보기 여섯 개가 뜬 적이 있다).
  조건이 따로 없었으면 빈 문자열로 둬라.
  예: "아이가 먹기 좋게 맵지 않고 자극이 적은 것만 담았어요."
  예: "기름지지 않고 담백한 쪽으로, 튀김과 밀가루는 뺐어요."

아래 JSON 객체만 출력해라. 다른 텍스트는 출력하지 마라.
예: {{"summary": "아이가 먹기 좋게 맵지 않은 것만 담았어요", "plan": [{{"n": 3, "why": "양파가 이틀 남았어요"}}, {{"n": 11, "why": "재료가 다 있어요"}}]}}
"""
...
def suggest(candidates, have, expiring, request_text="", days=PLAN_DAYS, model=None, hints=None):
    ...
    for i, c in enumerate(candidates):
        ...
        # 이 요리가 **새로 사게 하는 것**. 이름까지 적어 준다 — 개수만 적으면
        # 어느 것이 겹치는지(= 하나 사서 여러 날 쓰는지) 알 수가 없다.
        names = c.get("buy_names")
        if names:
            cost = f" · 새로 살 것: {', '.join(names[:6])}"
        elif isinstance(c.get("buy_extra"), int):
            cost = " · 새로 살 것 없음"
        else:
            cost = ""
