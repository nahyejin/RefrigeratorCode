...
- purchase_date: 영수증/주문내역의 결제·구매·주문 날짜. YYYY-MM-DD 형식. 안 보이면 null.
...
_RECEIPT_PROMPT = """이 이미지는 **산 것을 확인할 수 있는 화면**이다 — 마트/슈퍼 영수증일 수도
있고, 쿠팡·마켓컬리 같은 온라인 쇼핑몰의 주문내역/구매내역 화면을
캡처한 것일 수도 있다. 어느 쪽이든 구매한 식재료와 구매 날짜를 뽑아내라.

규칙:
- 식재료가 아닌 항목은 제외한다.
  영수증이면: 비닐봉투, 종량제봉투, 할인, 포인트, 적립, 합계, 부가세,
  카드번호, 매장명 등.
  온라인 주문내역이면: 배송 상태("배송완료"·"배송중"), 리뷰/별점, 쿠폰·할인
  문구, "로켓배송"·"무료배송" 같은 배지, 수량 조절 버튼, "재구매"·"리뷰쓰기"
  같은 UI 문구 등.
- 브랜드명, 용량, 수량, 규격, 가격, 상품 홍보 문구는 빼고 재료 이름만 남긴다.
  예) "CJ 백설 물엿 700g" -> "물엿"
  예) "무항생제 대파 1단" -> "대파"
  예) "[산지직송] 제주 감귤 3kg" -> "감귤"
- 같은 재료가 여러 줄이면 한 번만 적는다.
- 글자가 흐릿해 확신이 없으면 포함하지 않는다.
- 화면에 찍힌 결제일시·주문일자 등 구매 관련 날짜를 purchase_date 로
  적는다. 안 보이면 null.
- 영수증/주문내역에는 보통 유통기한이 없으므로 expiry 는 대개 null 이다.
...
_AUTO_PROMPT = """아래 이미지들은 마트 영수증, 온라인 쇼핑몰(쿠팡·마켓컬리 등) 주문내역
캡처, 또는 식재료 사진이다. 무엇인지 스스로 판단해서, 모든 이미지를 합쳐
담아야 할 식재료 이름만 하나의 목록으로 뽑아라.

영수증/온라인 주문내역이면:
- 식재료가 아닌 항목은 제외한다. 영수증은 비닐봉투, 종량제봉투, 할인, 적립,
  포인트, 합계, 부가세, 카드번호, 매장명 등. 온라인 주문내역은 배송 상태,
  리뷰/별점, 쿠폰 문구, 배송 배지, 수량 조절 버튼 등.
...
- 영수증/주문내역이 있으면 그 결제·주문 날짜를 purchase_date 로 적는다.
