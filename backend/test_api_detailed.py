...
# 필터링 API 테스트
api_url = "https://refrigeratorcode-production.up.railway.app/api/recipes/filter?match_rate_min=30&match_rate_max=100&sort_by=match_rate&size=20"

try:
    response = requests.get(api_url)
    if response.status_code == 200:
        data = response.json()
        print("=== 필터링 API 응답 상세 분석 ===")
