...
**제조사·브랜드명은 대표어에 남기지 마라.** `동원 7겹돈까스`, `비비고 사골곰탕 진`,
`목우촌 주부9단 구운마늘 프랑크 소시지` 처럼 제조사·브랜드가 붙은 상품명이 오면,
브랜드를 뗀 일반 명칭(`돈까스`, `사골곰탕`, `프랑크 소시지`)이 이미 사전에
있으면 그 재료의 **synonym** 으로 묶어라. 없으면 브랜드를 뗀 이름으로
**keyword**(새 재료)를 만들어라 — 브랜드명 자체를 대표어로 쓰지 마라. 여러
제품 특징이 줄줄이 붙어 뭐가 핵심 재료인지 판단이 안 서면(예: 상품 시리즈명 +
수식어 + 세부 규격이 뒤섞인 경우) skip 해라.

**성분·영양 표시는 국내산·유기농과 똑같이 뗀다.** `저당`, `저염`, `무가당`,
`제로`, `라이트`, `고단백` 같은 말은 재료의 성질이 아니라 가공 방식 표시다.
`저당 허니 머스타드` 는 `허니 머스타드`(사전에 있으면 그 동의어로, 없으면
그 이름으로 새 재료) 로 다뤄라 — `저당 허니 머스타드` 를 그대로 대표어로
만들지 마라.

**이름에 괄호로 다른 이름이 붙어 있으면 그 괄호 안 이름부터 사전에서 찾아라.**
`조청(물엿)` 처럼 괄호 안이 이미 사전에 있는 이름이면 그 재료의 synonym 으로
묶어라. 괄호 안도 사전에 없으면 괄호를 뗀 앞부분을 대표어로 keyword 를
만들어라 — 괄호를 포함한 문자열을 통째로 새 대표어로 만들지 마라.
