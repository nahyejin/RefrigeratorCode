...
`coupang_search_urls.txt` 가 생긴다. 많이 쓰이는 순으로, 아직 링크가 없는
재료만 들어 있다.

```
  1. 다진마늘           (레시피 11678개)
     https://www.coupang.com/np/search?q=다진마늘

  2. 파              (레시피 11540개)
     https://www.coupang.com/np/search?q=파
```

> **검색어가 너무 넓으면 `q=` 뒤를 고쳐서 만드세요.**
> `파` 로 검색하면 파김치·파스타가 섞여 나온다. `대파` 로 바꿔서 링크를
> 만들면 된다. 마찬가지로 `고추`→`청양고추`, `면`→`소면`, `다리`→`닭다리`.
>
> 바꾸는 건 **검색어뿐**이다. CSV 첫 칸(`ingredient_keyword`)은 사전 대표어와
> 맞아야 매칭되므로 **절대 고치지 마세요.**
