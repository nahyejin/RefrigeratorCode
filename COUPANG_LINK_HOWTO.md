...
재료마다 **검색어를 클립보드에 미리 넣어 준다.** 파트너스 검색창에서
`Ctrl+A` → `Ctrl+V` → `Enter`, 그리고 링크 복사 버튼을 누르면 끝이다.
**복사하는 순간** 그 줄에 적히고 저장된 뒤 다음 재료로 넘어간다.
건너뛰려면 Enter, 그만두려면 Ctrl+C — 그때까지 한 것은 이미 저장돼 있다.

```
[31/218]  물엿   (레시피 2294개)
        https://www.coupang.com/np/search?q=물엿
        검색어 '물엿' 를 클립보드에 넣었습니다 → 검색창에 Ctrl+V
        기다리는 중...
```

`--open` 을 붙이면 파트너스 검색 화면까지 브라우저로 열어 준다. 다만 재료마다
탭이 하나씩 쌓이므로, 한 탭에서 붙여넣기로 하는 편이 대개 편하다.
`--no-copy` 는 클립보드를 안 건드린다.

> 쿠팡 페이지 자체를 대신 조작해 주지는 못한다 — 자동화가 정책상 막혀 있다.
> 그래서 **붙여넣기와 복사 두 동작**까지가 줄일 수 있는 한계다.
