...
**250개를 반복해야 한다면 이걸 켜 두세요.** 파트너스에서 **검색하고 복사만**
하면 나머지를 알아서 한다 — CSV 를 열고, 그 재료 줄을 찾고, 붙여넣고, 저장하는
네 단계가 사라진다.

```bash
python scripts/collect_coupang_links.py --top 30
```

화면에 재료 이름과 검색 URL 이 하나씩 뜬다. 파트너스에서 그걸 검색해 링크 복사
버튼을 누르면, **복사하는 순간** 그 줄에 적히고 저장된 뒤 다음 재료로 넘어간다.
건너뛰려면 Enter, 그만두려면 Ctrl+C — 그때까지 한 것은 이미 저장돼 있다.

> **API 키를 받을 수 있으면 이것도 필요 없다.** 한 줄로 250개가 끝난다.
> 다만 API 키는 파트너스에서 **최종 승인된 회원만** 발급된다
> (`파트너스 API` 화면의 `생성` 버튼이 잠겨 있으면 아직 그 단계다).
>
> ```bash
> python scripts/generate_coupang_links.py --all --write
> ```
