...
>
> **시간대(2026-08-29)**: Railway MySQL 서버 자체 시계는 UTC지만, 백엔드
> `get_db()` 커넥션에 `SET time_zone = '+09:00'`을 걸어둬서 이 앱을 통해 쓰이는
> `NOW()`/`CURRENT_TIMESTAMP` 값은 전부 KST(한국시간) 기준입니다. 이전에 UTC로
> 쌓여있던 과거 데이터도 이날 한 번 +9시간 보정을 마쳤습니다. 즉 지금부터
> `SELECT`로 직접 보는 `created_at`/`updated_at` 값은 추가 보정 없이 그대로
> 한국 시간으로 읽으면 됩니다.
...
savings_per_meal        INT           NULL              -- 혼자일 때 한 끼당 절약액 추정치(원). NULL이면 8,000
...
- `household_id`~`savings_per_meal` 은 2026-08-27~28에 걸쳐 추가된 "식구
  그룹" 기능용 컬럼입니다. 자세한 동작(공유 판정, 목표/식구수/한끼추정액이
  개인값과 그룹값 중 어느 쪽을 쓰는지 등)은
...
savings_per_meal       INT           NULL               -- 그룹의 한 끼당 절약액 추정치(원). NULL이면 8,000
created_at             DATETIME      NOT NULL
```

- 그룹원 전체가 **동등한 권한**을 가진다 — `created_by`만 특별한 권한을 갖지
  않으며, `allow_ingredient_merge`/`monthly_cooking_goal`/`family_size`/
  `savings_per_meal` 모두 그룹원 누구나 바꿀 수 있다.
