...
- 단계별 안내 (내냉장고 3단계 → 냉장고요리 8단계, 총 11단계)

**가이드가 가리킬 요소를 만들 때 지켜야 할 것 — 노란 하이라이트가 엉뚱한 데
찍히는 사고가 반복해서 났다.**

1. **반드시 `data-guide-target="..."` 로만 찾는다.** 화면 문구(placeholder,
   title)로 찾으면 문구를 다듬는 순간 조용히 안 맞게 된다. 실제로 입력창
   placeholder 를 "추가할 재료명을 입력하세요" → "재료명을 입력하세요" 로
   줄였더니 그 단계의 하이라이트가 통째로 사라졌다.
2. **속성은 사용자가 "누르라"고 안내받는 것 전체에 붙인다.** 재료 배지 안내인데
   속성이 안쪽 이름 `<span>` 에 붙어 있어서, 노란 테두리가 글자에만 걸리고
   D-표기·X 는 밖에 남았다.
3. **늦게 생기는 요소도 찾아 준다.** `RecipeCard` 는 썸네일이 실제로 뜨는지
   확인될 때까지 `null` 을 돌려주므로, 가이드가 시작될 때 즐겨찾기·완료·공유·기록
   버튼은 아직 문서에 없다. 오버레이가 단계가 떠 있는 동안 계속 다시 찾는다.
4. **화면 밖 요소는 끌어온다.** 오버레이가 안전 구간(고정 헤더 아래 ~ 하단 탭 위)
   가운데로 스크롤한 뒤에 위치를 잰다. 예전엔 스크롤을 안 해서, 화면 아래에 있는
   타겟이 높이 0 으로 잘리고 최소 크기 상자가 **화면 맨 위**에 그려졌다.
