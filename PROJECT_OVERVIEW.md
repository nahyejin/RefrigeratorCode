...
**정규화는 3단계입니다** (`llm_ingredient_extraction._resolve_canonical`).
어느 단계든 **사전에 실제로 있는 결과만** 채택하므로 없는 재료를 지어내지 않습니다.

| 단계 | 하는 일 | 예 |
|---|---|---|
| 1 | 완전일치 | `계란` → `달걀` |
| 2 | 손질 수식어 제거 | `다진 생강` → `생강`, `신김치` → `김치` |
| 3 | 띄어쓰기 조각 | `다시다 감칠맛` → `다시다`, `돼지고기 목살` → `목살` |

3단계는 한국어가 뒤쪽을 핵심어로 삼는 걸 이용합니다 — 뒤에서 시작하는 조각을
먼저 채택하되, 뒤쪽이 사전에 없으면 앞쪽을 씁니다.

**대표어가 또 다른 대표어의 별칭인 연쇄는 끝까지 따라가 접습니다.**
(`볶은참깨 → 통깨` 인데 `통깨 → 참깨` 이면 `볶은참깨 → 참깨`)
같은 재료가 표기에 따라 다른 pill 로 갈라지면 냉장고 매칭이 어긋나기 때문입니다.

> ⚠️ **룰베이스 배치(`update_used_ingredients_batch.py`)는 자체적으로 사전을
> 만들면서 연쇄를 풀지 않습니다.** 그래서 연쇄는 코드가 아니라 **CSV 데이터
> 자체**에서 정리해 두었습니다. 사전에 별칭을 추가할 때 그 대표어가 또 다른
> 행의 동의어로 잡혀 있지 않은지 확인해 주세요.

사전을 고친 뒤에는 아래 두 스크립트를 돌려 이미 저장된 값도 맞춰 줍니다.
...
- LLM 은 무료 티어 한도(하루 500회 호출) 안에서 **12건씩 묶어** 하루 약 5,280건 처리합니다.
  (440회 사용 — 챗봇·이미지 인식과 키를 공유할 때를 대비해 60회를 남겨 둔 값)
- **일일 한도가 소진되면 즉시 중단합니다.** 한도는 자정(PT)까지 안 풀려서 재시도
  백오프가 전부 헛돌기 때문입니다. 다만 429 의 `quotaId` 가 *분당* 제한이라고
  명시한 경우는 중단하지 않습니다 — 곧 풀릴 제한을 일일 한도로 오판하면 그날
  작업을 통째로 조기 종료시키게 됩니다.
...
- 유통기한/구매일 관리
- 저장 버튼 (로그인 사용자만)
- **사진으로 재료 담기** — 영수증을 찍거나 앨범에서 고르면(최대 5장) 재료와
  구매일자를 읽어 준다. 담기 전에 사용자가 검토·수정한다.
  자세한 동작은 `INGREDIENT_RECOGNITION_FEATURE.md` 참고

### 내 냉장고 재료는 어디에 저장되나 (계정 전환 주의)

재료는 **두 곳**에 있고, 페이지마다 읽는 곳이 다르다. 이 차이 때문에 실제로
버그가 났었으므로 고칠 때 반드시 함께 본다.

| 저장소 | 키 | 누가 읽나 |
|---|---|---|
| DB | `user_ingredients` (user_id별) | 내 냉장고 페이지 (로그인 시 우선) |
| localStorage | `myfridge_ingredients` (**계정 구분 없음, 한 칸**) | 냉장고 요리, 요즘 인기, 챗봇, 정렬바 |

내 냉장고 페이지는 로그인 사용자면 DB 를 먼저 읽고, 성공하면 그 값을
localStorage 에도 덮어써 둘을 맞춘다. 나머지 화면은 **localStorage 만** 읽는다
(`getMyIngredients()`).

#### 계정이 바뀌면 이전 재료가 남는 문제

`myfridge_ingredients` 는 계정별로 나뉘어 있지 않은데 **지우는 건 로그아웃할
때뿐**이었다. 그래서 비회원으로 쓰다가 가입하거나 다른 계정으로 들어오면
이전 재료가 그대로 남았고,

- 내 냉장고는 DB(비어 있음)를 보여주는데
- 냉장고 요리 카드에는 매칭률이 100%/65% 처럼 떴다

→ `AuthContext.clearDataOfOtherAccount()` 가 `last_active_user_id` 와 비교해
**계정이 바뀐 경우에만** 냉장고 재료·마이페이지 목록·레시피 목록 캐시를 지운다.
토큰 로그인(소셜 리다이렉트 포함)과 세션 복원 양쪽에서 호출한다. 같은 계정이면
아무것도 지우지 않는다.

> ⚠️ localStorage 를 읽어 쓰는 화면을 새로 만들 때는 이 점을 기억할 것.
> 재료를 **DB 에서 직접** 읽는 게 아니라면 계정 전환 시 오염될 수 있다.

#### 기본 재료 자동 투입

처음 쓰는 사람이 텅 빈 냉장고를 보지 않도록 기본 재료(실온 11 / 냉장 17 / 냉동 3)를
넣어 준다. 조건은 `MyFridge.tsx` 에 있다.

- 재료가 비어 있고
- 이 계정에 아직 넣은 적이 없고 (`myfridge_seeded_{userId}`, 비회원은 `_guest`)
- 비회원이거나, 회원이면 **DB 조회를 마쳤고** 로컬에도 없을 때

두 번째 조건이 없으면 사용자가 일부러 전부 지웠을 때 새로고침만 해도 기본 재료가
되살아난다. 세 번째 조건은 예전에 "DB 로드가 **실패**했을 때" 로만 돼 있어서,
갓 만든 계정처럼 **조회는 성공했는데 0건** 인 경우가 통째로 빠져 있었다
(로그인해도 냉장고가 비어 있던 원인).
