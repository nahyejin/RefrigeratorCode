...
# 두 프롬프트(단일·묶음)가 **함께 쓰는 규칙.** 한 곳에만 적는다.
#
# 전에는 같은 46줄이 두 군데에 복사돼 있었다(55줄 중 84%). 판정 기준을
# 고칠 때 한쪽만 고치면 **조용히 어긋난다** — 한 건씩 처리할 때와 묶어서
# 처리할 때가 서로 다른 기준으로 같은 카탈로그를 판정하게 된다. 실제로
# "옥수수 삶는법" 을 거르는 규칙을 넣을 때 두 곳을 다 고쳐야 했고,
# 두 곳인 줄 몰랐으면 절반만 적용됐을 것이다.
#
# 두 프롬프트가 다른 것은 **머리말·본문 넣는 자리·출력 형식**뿐이다.
# 그 셋만 각자 갖고, 규칙은 아래 둘을 함께 쓴다.
_RULES_INGREDIENTS = """- 재료 이름만 적는다. 수량, 단위, 손질법, 괄호 설명은 빼고 순수 재료명만.
- **이미 만들어진 음식도 재료다.** 그 요리에 넣었으면 이름 그대로 적어라.
  ("김치찌개" 에 넣은 `김치`, "부대찌개" 에 넣은 `햄`·`라면사리`,
   "김밥" 에 넣은 `계란말이`·`단무지`, "떡볶이" 에 넣은 `어묵`·`군만두`,
   "빙수" 에 올린 `아이스크림`)
  요리 이름처럼 보인다고 빼지 마라 — 사는 물건이고 냉장고에 있는 것이다.
  다만 **지금 만들고 있는 그 요리 자신**은 재료가 아니다
  ("김치찌개 만드는 법" 의 재료에 `김치찌개` 를 넣지 마라).
...
- **요리를 만드는 글이 아니면** 재료 대신 문자열 "NOT_RECIPE" 를 출력한다.
  ...
  **재료 하나를 익히거나 다듬기만 하는 글도 요리 글이 아니다.**
  ("옥수수 삶는법", "감자 삶는 시간", "브로콜리 데치는 법", "갈치 손질법")
  익히는 과정이 본문에 있어도 그것만으로는 **한 끼가 되지 않는다** —
  이런 글이 식단에 올라오면 그 날 뭘 먹는지가 안 정해진다.
  다만 **본문에 한 끼로 먹을 요리를 완성하는 과정이 있으면** 맛집 이야기나
  제품 홍보가 섞여 있어도 요리 글로 본다.
  ("이 양념으로 제육볶음을 만들었다" 는 요리 글, "이 양념 사 봤는데 맛있다" 는 아니다)
...
_RULES_OUTPUT = """- **재료 상세**(`ingredients_detail`)를 분량과 함께 그대로 적는다.
...
PROMPT_TEMPLATE = (
    """너는 레시피 본문에서 **재료와 조리 순서**를 뽑아내는 어시스턴트다.
...
BATCH_PROMPT_TEMPLATE = (
    """너는 여러 개의 레시피 본문 각각에서 **재료와 조리 순서**를 뽑아내는 어시스턴트다.
