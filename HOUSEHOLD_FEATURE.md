...
1. **목표 카드** (회색 배경): 이번 달 목표·달성률 게이지(그룹이면 완료 많은
   사람 순으로 색을 나눠 채움)·절약액까지는 항상 보이고, 인원별 범례와 안내
   문구는 "자세히 보기" 버튼으로 접혀 있다(기본 접힘 — 펼쳐 두면 카드가
   길어져서 달력이 한 화면에 안 들어오는 문제가 있었음).
...
- 멤버 목록에서:
  ...
  - 비공개인 다른 사람 행: 행 전체가 버튼 — 누르면 그 사람 활동 통계를 보고
    "공유 요청하기"를 보낼 수 있는 팝업이 뜬다.
