...
### 남은 작업
1. **네이버 로그인 검수요청 제출·승인** — CookMatch 앱이 아직 "개발 중" 상태(2026-09-18 확인). 이 상태에서는 "멤버관리"에 등록한 테스터 계정만 로그인되고, 실제 신규 사용자는 전부 막힘. developers.naver.com → CookMatch → **"네이버 로그인 검수요청"** 버튼 제출 → 승인이 나야 일반 사용자도 로그인 가능. **앱스토어에 실사용자 대상으로 출시하기 전 반드시 먼저 끝내야 하는 단계.**
2. **네이티브 앱용 소셜 로그인(구글/카카오/네이버) 딥링크 처리** — 지금 등록한 서비스 URL/Callback URL은 **웹 브라우저 로그인 전용**. 네이티브 앱(안드로이드/iOS)은 앱 내부가 인터넷 주소가 아니라 로컬 번들(`capacitor://localhost` 등)이라 이 방식을 그대로 못 씀 — 별도로 네이버 개발자센터의 "Android 앱"/"iOS 앱" 환경(패키지명+서명 키 해시, Bundle ID로 등록하는 방식)을 추가하고, 로그인 후 앱으로 돌아오는 딥링크 코드를 붙여야 함. **기존 웹 설정을 바꾸는 게 아니라 새 환경을 추가하는 작업** — 앱스토어에 실제로 빌드·출시하는 시점에 진행. 아직 구현 안 됨.
