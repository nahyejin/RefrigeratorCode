...
### 남은 작업
1. **네이버 로그인 검수요청 제출·승인 필요** — 네이버 개발자센터에서 CookMatch 앱이 아직 "개발 중" 상태(2026-09-18 확인). 이 상태에서는 "멤버관리"에 등록한 테스터 계정만 로그인 가능하고, 실제 일반 사용자는 전부 "등록되지 않은 사이트에서 로그인을 시도했습니다" 에러로 막힘. 검수요청 제출 → 승인이 앱스토어 실사용자 대상 출시 전 필수.
2. **네이버 로그인 실제 동작 확인** — `BACKEND_URL` 수정 + 네이버 콜백 등록 후, 재배포된 상태에서 네이버/구글/카카오 로그인 셋 다 실제로 되는지 확인 필요.
