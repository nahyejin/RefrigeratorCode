...
## 📅 2026-09-23 현재 상태
- **iOS 1.0.2 빌드 8 업로드 완료(2026-09-23 17:22, 맥)**: 빌드 7 로 심사 대기 중이던 것을 1.0.3 수정 모음(`c673fefb`)까지 넣어 교체. → 제출 취소·빌드 8 교체 후 **다시 심사 제출 완료**(「1개의 항목 제출됨」). 자동 출시·모든 사용자에게 즉시·평점 유지. 남은 것: 심사 결과 메일 → 통과 시 아이폰에서 쿠팡 배너·위젯 확인.
...
- **Play 프로덕션 액세스 체크리스트**: ① 비공개 테스트 버전 게시 ✅ ② **12명 이상 테스터 참여 ✅** ③ 12명 이상으로 14일 이상 실행 ⏳ → 「프로덕션 신청」 버튼은 아직 회색. 12명이 모인 날이 2026-09-22 이므로 **2026-10-06 경 신청 가능**. 그때까지 트랙·테스터를 그대로 둔다.
- **iOS 1.0.1 (빌드 6)**: 2026-09-22 제출 → **2026-09-23 심사 통과**(Submission ID 0720ea48-…). 자동 출시라 최대 24시간 내 공개. 반영되면 확인: ① 설명이 「오늘 저녁 뭐 해 먹지?」로 시작 ② Language KO ③ 「쿡매치」·「냉털이」 검색(며칠 걸릴 수 있음). 다음 iOS 는 **1.0.2 (빌드 7)**.
- **테스터 의견 통로**: 비공개 테스트 트랙의 「의견 URL 또는 이메일」이 **비어 있다** → `920803hj@gmail.com` 을 넣을 것. BETA FLOW 테스터는 설치·실행 위주라 의견을 기대하기 어렵다. 지인 안드로이드 사용자 2~3명을 테스터 목록에 추가해 실제 의견을 받고 `RELEASE_NOTES.md` 「테스트 기간 기록」에 남긴다(프로덕션 신청서 답변 재료).
- **문서**: 릴스 소재·제미나이 프롬프트 = [store/REEL_PLAN.md](store/REEL_PLAN.md), 프로덕션 신청서 초안 = [store/PLAY_PRODUCTION_APPLICATION.md](store/PLAY_PRODUCTION_APPLICATION.md)(제출 전 〔 〕 값 교체).
- **테스트 기간 중 업데이트**: 1.0.2 에 넣을 수정이 이미 5건 쌓임(RELEASE_NOTES) → 10월 초 전에 안드로이드·iOS 한 번 올리면 신청에 유리.
- **App Store 「연령 등급 소셜 미디어 질문」 안내**: 애플 공통 공지. 앱 정보 화면에서 문항이 보이면 답할 것(쿡매치는 SNS 기능 없음 → 대부분 아니요).
...
- iOS 1.0.1 은 빌드 6 으로 교체해 재제출 — ⚠ 「심사에 추가」만 하고 「제출 초안 → 심사를 위해 제출」을 안 눌러 몇 시간 「심사 준비됨」에 머물렀다. **17:08 실제 제출 완료(1.0.1 (6))**. 다음부터 상태가 「심사 대기 중」인지 꼭 확인.
