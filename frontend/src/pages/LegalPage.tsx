 ...
 * 2026-09-21 개정: iOS 앱의 Apple 로그인(Sign in with Apple)을 로그인 방식·맡기는 곳에 추가했다.
 *   같은 날 「Apple 로그인 연결 정보」(서버에 두는 Apple 토큰)와, 탈퇴할 때 Apple 에 연결 취소를 요청한다는 것을
 *   추가했다(App Store 심사 가이드라인 5.1.1(v)). 코드: backend/app.py `revoke_apple_token_for_user`.
 *   ⚠️ 이 문장은 Railway 에 APPLE_TEAM_ID·APPLE_SIGNIN_KEY_ID·APPLE_SIGNIN_PRIVATE_KEY 가 있어야 사실이다
 *   (없으면 토큰 저장·취소를 건너뜀 — backend/apple_signin.py).
 * 2026-09-20 개정: 로그인한 사람의 「AI 대화 기록」(지난 대화 최대 10개)을 서버에 두기 시작해 표에 추가했다.
 *   (이용자가 요청한 기능 — 다른 기기에서도 지난 대화를 보기 위함. 아직 이용자가 거의 없어 공고일=시행일.)
 * 새로 받기 시작한 것이 아니라 이미 하던 일을 빠뜨렸던 것을 바로잡은 것이라(이용자에게
 * 불리한 변경이 아님) 공고일과 시행일을 같은 날로 둔다.
 */
// 2026-09-22 개정: 탈퇴 계정 보관 기간을 「탈퇴 후 1년 뒤 삭제, 요청 시 7일 안에 삭제」로 명시하고
//   계정 삭제 안내(/account-deletion) 링크를 추가했다(Google Play 계정 삭제 URL 요건). 이전엔 기간 없이
//   "요청하면 지운다"였으므로 이용자에게 불리한 변경이 아니라 공고일=시행일.
const PRIVACY_UPDATED_AT = '2026-09-22';
const PRIVACY_EFFECTIVE_AT = '2026-09-22';
  ...
  <>
    ...
    <table style={S.table}>
      ...
      <tbody>
        <tr>
          <td style={S.td}>계정</td>
          <td style={S.td}>이메일, 닉네임, 로그인 방식(구글/카카오/네이버/애플/이메일), 비밀번호(이메일로 가입한 경우만, 되돌릴 수 없게 암호화)</td>
          <td style={S.td}>로그인, 내 재료·기록을 다음에도 보여 주기</td>
        </tr>
        <tr>
          <td style={S.td}>Apple 로그인 연결 정보</td>
          <td style={S.td}>Apple 로그인(iOS 앱)으로 들어온 경우, Apple 이 발급한 로그인 연결용 토큰. 이 토큰으로는 Apple 계정의 다른 정보를 볼 수 없고, 회원 탈퇴 때 Apple 에 연결 취소를 요청하는 데에만 씁니다</td>
          <td style={S.td}>회원 탈퇴 때 Apple 계정과의 연결 끊기</td>
        </tr>
        <tr>
        ...
        <tr>
          <td style={S.td}>AI 대화 기록</td>
          <td style={S.td}>AI 식단 추천 대화의 지난 대화 최대 10개(내가 쓴 조건, 받은 식단의 제목·링크·썸네일). 로그인한 경우만 서버에 두고, 비회원은 이 기기에만 남습니다</td>
          <td style={S.td}>다른 기기·앱에서도 지난 대화와 받은 식단을 다시 보기</td>
        </tr>
        <tr>
    ...
    <table style={S.table}>
      ...
      <tbody>
        <tr>
          <td style={S.td}>Google · 카카오 · 네이버 · Apple (소셜 로그인)</td>
          <td style={S.td}>
            로그인할 때 이메일과 이름(닉네임)을 받아 옵니다. 카카오·네이버가 이메일을 주지
            않으면 서비스 안에서만 쓰는 임시 주소로 계정을 만듭니다. Apple 로그인(iOS 앱)은
            이름을 처음 한 번만 주고, 이메일 가리기를 고르면 Apple 이 만든 가림 주소를 줍니다
          </td>
        </tr>
        <tr>
    ...
    <ul style={S.ul}>
      ...
      <li style={S.li}>
        회원 탈퇴를 하면 계정을 <b>즉시 사용할 수 없게</b> 하고, 냉장고 재료·요리 기록·식단·AI 대화
        기록·알림 수신 정보를 <b>바로 지웁니다.</b> 같은 이메일로 다시 가입해 가입 혜택을 여러 번 받는
        경우를 가려내기 위해 계정 기록(이메일·닉네임·로그인 방식·탈퇴 시각)과 가입 혜택·이용량 기록만
        <b>탈퇴 후 1년 동안 보관한 뒤 삭제</b>합니다. 그 전에 완전한 삭제를 원하시면 아래 연락처로
        요청해 주세요 — 요청일로부터 7일 안에 지웁니다.
        (방법·삭제되는 항목: <a href="/account-deletion">계정 삭제 안내</a>)
      </li>
      <li style={S.li}>
        Apple 로그인으로 들어온 분이 회원 탈퇴를 하면, <b>Apple 에도 이 앱과의 연결을 끊어 달라고 요청</b>하고
        서버에 둔 Apple 로그인 연결 정보를 지웁니다. (Apple 계정 설정의 「로그인에 Apple 사용」 목록에서도 쿡매치가 사라집니다.)
      </li>
      <li style={S.li}>이용 기록(어떤 화면에서 무엇을 눌렀는지)은 서비스 개선에만 쓰고, 누구인지 알 수 없게 다룹니다.</li>
      <li style={S.li}>AI 대화 기록은 최근 10개까지만 갖고 있고, 앱에서 지난 대화를 지우면 서버에서도 지웁니다.</li>
