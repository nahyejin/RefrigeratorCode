 ...
 *
 * 실제 동작과 반드시 같아야 한다:
 *   - 회원 탈퇴(`/api/auth/delete-account`)는 계정을 쓸 수 없게 표시(deleted_at)만 하고 데이터는
 *     그대로 둔다(재가입 가입 혜택 중복 방지). 가족 그룹 냉장고는 남은 식구에게 넘기고, Apple 로그인이면
 *     Apple 연결을 끊는다.
 *   - 탈퇴 후 1년이 지나면 `scripts/purge_deleted_accounts.py`(매일 배치)가 계정과 데이터를 지운다.
  ...
  return (
    <div style={S.page}>
      ...
      <table style={S.table}>
        ...
        <tbody>
          <tr>
            <td style={S.td}>탈퇴 즉시</td>
            <td style={S.td}>
              계정으로 더 이상 로그인할 수 없습니다. 가족 그룹에서 빠지며, 함께 쓰던 냉장고는 남은
              식구에게 넘어갑니다. Apple 로그인으로 가입했다면 Apple 과의 연결도 끊습니다.
            </td>
          </tr>
          <tr>
            <td style={S.td}>탈퇴 후 1년이 지나면 자동으로 모두 삭제</td>
            <td style={S.td}>
              계정 정보(이메일·닉네임·로그인 방식), 냉장고 재료, 즐겨찾기·요리 기록·완료 기록·식단 계획,
              AI 대화 기록, 알림 수신 정보(기기 알림 주소), 이용·사용량 기록
            </td>
          </tr>
          <tr>
            <td style={S.td}>1년 전에 지워 달라고 요청하면</td>
            <td style={S.td}>위 데이터를 요청일로부터 7일 안에 모두 삭제합니다.</td>
          </tr>
        </tbody>
      </table>
      <ul style={S.ul}>
        <li style={S.li}>
          <b>왜 1년 동안 보관하나요?</b> 같은 이메일로 탈퇴와 재가입을 반복해 가입 혜택(AI 이용 크레딧)을
          여러 번 받는 것을 막기 위해서입니다. 보관하는 동안 이 데이터는 다른 목적으로 쓰지 않습니다.
        </li>
        <li style={S.li}>
