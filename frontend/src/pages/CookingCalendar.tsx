...
// 색은 빨간 펜(2026-09-20, "주황이 아니라 빨간색으로"). 멤버 점 색에는 빨강·노랑을 넣지 않아
// 이 표식과 헷갈리지 않는다.
const PLAN_MARK_STROKE = '#E5383B';
...
// 브랜드 강조색(노랑)·"요리 계획 있는 날" 표식(빨강)과 헷갈리지 않게 빨강·노랑 계열은 뺐다.
const MEMBER_COLORS = [
  '#3B82F6', // 파랑
  '#F97316', // 주황
  '#22C55E', // 초록
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', paddingTop: 72, paddingBottom: 84 }}>
      ...
      {/* 쿠팡 배너 — 화면 **맨 끝**, 달력 카드 **바깥**. 2026-09-22 에 월 목표 카드 바로 아래(화면 가운데)로 올렸더니
          목표 카드와 달력 사이에 광고가 끼어 "엄청 헷갈린다"는 지적(2026-09-26)으로 다시 맨 아래로. 위 내용과 넉넉히 띄운다. */}
      <div style={{ margin: '40px 14px 0' }}>
        <CoupangDynamicBanner height={50} style={{ marginBottom: 8 }} />
      </div>
      </div>
      </PullToRefresh>
