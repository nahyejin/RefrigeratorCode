...
// "계속 올라가는 중"인 느낌을 유지. 원본 그대로(1.0s) 쓰니 "너무 빨리 넘어간다"는 지적이 있어서,
// ffmpeg `setpts=2.3*PTS`로 프레임 수는 그대로 두고 재생 타임라인만 2.3배 늘려(60fps 원본이지만
// Remotion에는 rate=1로만 재생 — 재생속도 자체를 Remotion에서 조절하면 걸리는 분수 배속 버그를
// 피하기 위해 소스 파일 자체를 미리 늘려서 저장) 카운터를 천천히, 충분히 읽을 수 있게 함.
const SPLASH_VIDEO = "reel7_splash.mp4";
const SPLASH_LEN = 69; // 2.3s — setpts로 늘린 파일 전체(카운터가 아직 다 오르지 않은 채로 끝남)
