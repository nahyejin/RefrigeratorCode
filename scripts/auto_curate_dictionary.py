...
왜 전부 자동으로 하지 않나 — **꼬리가 쓰레기다**:
    ...
    그래서 **10회 이상**만 자동으로 돌린다(기본값, 2026-09-10). 한동안 2회로
    낮춰 뒀다가, 그 기준에서 통과한 것 중 `동원 7겹돈까스`·`저당 허니 머스타드`
    ·`조청(물엿)` 처럼 브랜드명·성분 표시·괄호 별칭이 그대로 새 재료가 된 것이
    발견돼 다시 올렸다(판정 규칙은 `dictionary_curation._PROMPT` 도 같이 고쳤다).
    10회 이상은 목록을 눈으로 훑어보면 거의 다 진짜 재료다 — `흰쌀밥`, `편마늘`,
    `청홍고추`, `슈레드치즈`, `계란지단`, `치아씨드`, `케이퍼`. 나머지 꼬리는
    어드민에 그대로 남아 필요하면 손으로 볼 수 있다.
...
#
# 처음에는 6회 -> 2회로 내렸다(횟수는 "이게 재료인가" 와 상관없고, 판정은
# LLM 이 `skip` 으로 이미 거르고 있다고 봤다). 그런데 2026-09-08 승인분을
# 사람이 다시 훑어보니 `동원 7겹돈까스`·`비비고 사골곰탕 진`·`목우촌 주부9단
# 구운마늘 프랑크 소시지` 같은 **브랜드 상품명**과 `저당 허니 머스타드`
# 같은 **성분 표시가 붙은 이름**, `조청(물엿)` 같은 **괄호 별칭**이 그대로
# 새 재료로 들어가 있었다. LLM 의 skip 판정은 "재료가 아닌 것"은 잘 걸렀지만
# "브랜드/성분이 붙어 대표어로 부적절한 이름"은 걸러야 한다고 명시하지
# 않아 통과시켰다(그 판정 규칙은 `dictionary_curation._PROMPT` 에서 따로
# 손봤다). 판정 자체가 불완전했던 상태에서 자동 통과 건수만 늘려 온
# 셈이라, 다시 10회로 올린다 — 10회 이상은 거의 다 진짜 재료이고
# (2026-09-10 기준 8종/144회만 해당), 애매한 꼬리는 어드민 '사전' 탭에서
# 사람이 보게 한다.
DEFAULT_MIN_HITS = 10
