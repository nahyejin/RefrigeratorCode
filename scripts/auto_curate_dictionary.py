...
왜 전부 자동으로 하지 않나 — **꼬리가 쓰레기다**:
    ...
    그래서 **2회 이상**만 자동으로 돌린다(기본값, 2026-09-10 재조정). 그 위는
    목록을 눈으로 훑어보면 거의 다 진짜 재료다 — `흰쌀밥`, `편마늘`, `청홍고추`,
    `슈레드치즈`, `계란지단`, `치아씨드`, `케이퍼`. 나머지 꼬리는 어드민에 그대로
    남아 필요하면 손으로 볼 수 있다.

    (한때 10회까지 올렸었다. 2회 기준으로 통과한 것 중 `동원 7겹돈까스`·
    `저당 허니 머스타드`·`조청(물엿)` 처럼 브랜드명·성분 표시·괄호 별칭이
    그대로 새 재료가 된 게 발견돼서다. 그런데 10회로 올리면 2~9회 구간
    수백 종이 자동 처리를 못 받고 그대로 어드민 대기열에 쌓여 — 그만큼을
    사람이 매번 손으로 봐야 한다는 뜻이라 부담이 너무 컸다. 원인은 횟수가
    아니라 **판정 규칙의 빈틈**이었으므로, 프롬프트에 세 규칙을 추가하고
    (`dictionary_curation._PROMPT`), 결과로 나온 대표어 자체를 다시 검사하는
    `dictionary_curation.polluted_keyword_reason()` 을 추가했다 — 모델이
    프롬프트를 어겨도 브랜드·성분·괄호가 남은 대표어는 여기서 걸러져
    `skip` 으로 떨어진다. 안전장치를 대표어 쪽에 세웠으니 횟수 기준은
    다시 낮춰도 된다.)
...
# **몇 번 나온 이름부터 자동으로 넣을 것인가.**
#
# 6회 -> 2회 -> (브랜드/성분 오염 9건 발견) -> 10회 -> 다시 2회, 하루 사이에
# 두 번 뒤집혔다. 10회로 올리면 품질은 지켜지지만 2~9회 구간 수백 종이
# 전부 어드민 대기열로 쌓여 사람이 매번 손으로 봐야 했다 — 그게 자동화를
# 만든 이유 자체를 없앴다. 문제는 횟수가 아니라 **판정이 브랜드/성분/괄호를
# 걸러야 한다는 규칙이 없었던 것**이었으므로, 프롬프트를 고치고
# (`dictionary_curation._PROMPT`) 결과 대표어를 다시 검사하는 이중 안전장치
# (`dictionary_curation.polluted_keyword_reason`) 를 추가한 뒤 2회로 되돌렸다.
# 이제 모델이 규칙을 어겨도 대표어 자체가 걸러지므로, 횟수 기준을 낮게 둬도
# 안전하다.
DEFAULT_MIN_HITS = 2
...
#
# 기준을 2회로 내리면서 대상이 465종 -> 3,756종이 됐다. 60회로는 하루에
# 1,800종밖에 못 보므로 상한을 올린다. 밀린 것을 이틀이면 소화하고, 그 뒤에는
# 하루에 새로 생기는 몇십 종만 남아 상한에 닿을 일이 없다.
