...
#
# 처음에는 6회로 잡았다. 꼬리에 `코스트코 호래기`·`쿠킹 포일` 같은 것이 섞여
# 있어서 조심한 것이었다. 그런데 한 사이클을 돌려 보니 **그 판정은 LLM 이 이미
# 하고 있었다** — 1,418종 중 465종을 `skip` 으로 떨어뜨렸고 그 안에 가게 이름·
# 영수증 항목·알 수 없는 글자가 들어 있었다. 횟수는 "이게 재료인가" 와 별
# 상관이 없다. `트러플오일` 이 세 번 나왔다고 재료가 아닌 것이 아니다.
#
# 6회로 두면 **3,291종(누적 9,417회)이 영영 대기**하고, 그만큼 카드의 재료가
# 비고 매칭률이 낮게 잡힌다. 그래서 2회로 내린다.
#
# **1회는 뺀다.** 오타와 한 번 스쳐 간 말이 대부분이라, 사전에 한 번 들어가면
# 되돌리기 어려운 것에 비해 얻는 게 적다. 두 번 이상 나왔다는 건 적어도
# 서로 다른 글에서 같은 말이 쓰였다는 뜻이다.
DEFAULT_MIN_HITS = 2
